...
function ButtonUpdateCart({ products = [], id }: IProps) {
  ...
  const { mutate } = useMutation({
    ...
    async onSuccess(data) {
      const key = ["cart", id]
      const getCacheProfile = queryClient.getQueryData<AxiosResponse<ICart, any, {}>>(key)

      console.log("getCacheProfile: ", getCacheProfile)
      console.log("data: ", data?.data)

      if (data?.status === 200 && data?.data && getCacheProfile) {
        const newData = {
          ...getCacheProfile,
          data: data?.data,
        }

        queryClient.setQueryData<AxiosResponse<ICart, any, {}>>(key, newData)
        await queryClient.refetchQueries({
          predicate: (query) => {
            console.log("query: ", query?.queryKey?.[0] === "carts")

            return query.queryKey[0] === "carts"
          },
        })
      }
