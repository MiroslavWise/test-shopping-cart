...
import { ARRAY_LIMIT } from "@/constants/limit"
import { dispatchLimit } from "@/stores/pages"
...
function Selector({ limit }: IProps) {
  ...
  return (
    <div className="h-8 pr-1 flex flex-row flex-nowrap items-center justify-between pl-2.5 rounded-md border border-[#0000001a] relative">
      <span className="text-sm font-normal text-black/80" dangerouslySetInnerHTML={{ __html: limit }} />
      <div
        className={cx(
          "w-full flex flex-col absolute top-9 left-0 right-0 rounded-md border border-[#0000001a] shadow-md z-50 bg-white p-1",
          open ? "opacity-100 visible pointer-events-auto" : "opacity-0 invisible pointer-events-none",
        )}
      >
        {ARRAY_LIMIT.map((item) => (
          <a
            className={cx(
              "w-full text-sm font-normal text-black/80 text-start py-1 px-2 rounded-md transition-colors",
              item === limit ? "bg-[#0000000d]" : "bg-white hover:bg-[#0000000d]",
            )}
