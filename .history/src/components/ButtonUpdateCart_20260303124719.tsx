...
function ButtonUpdateCart({ products = [], id }: IProps) {
  ...
  return (
    <ButtonUpdate className="font-medium text-sm border border-black/10" onClick={onOpen}>
      <span>Изменить</span>
      <Popover
        className={cx(
      ...
      >
        ...
        <ButtonEnd className="ml-auto font-medium text-sm" dangerouslySetInnerHTML={{ __html: "Обновить" }} onClick={update} />
      </Popover>
    </ButtonUpdate>
  )
}
