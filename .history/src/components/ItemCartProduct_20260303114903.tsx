...
function ItemCartProduct({ thumbnail, title, price }: IProps) {
  return (
    <Li>
      <ImgDiv className="bg-black/15">
        <img src={thumbnail} width={"100%"} height={"auto"} className="w-full aspect-video h-auto object-cover" />
      </ImgDiv>
      <div className="w-full flex flex-col p-4 border-x border-black/15">
        <div className="w-full flex flex-row items-start justify-between gap-2">
          <p className="text-base leading-snug font-medium text-black/90">{title}</p>
          <div className="text-black/90 whitespace-nowrap text-xs py-0.5 px-2 flex items-center bg-gray-200 rounded-full">{price}</div>
        </div>
      </div>
