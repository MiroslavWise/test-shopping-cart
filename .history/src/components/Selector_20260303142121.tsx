...
function Selector({ limit }: IProps) {
  ...
  return (
    <div className="h-8 pr-1 flex flex-row items-center justify-between pl-2.5 rounded-md border border-[#0000001a]">
      <span className="text-sm font-normal text-black/80">100</span>
      <button type="button" className="relative" onClick={handle}>
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          fill="none"
          className={cx("absolute w-6 h-6 aspect-square top-1/2 left-1/2 -translate-1/2 transition-all", open ? "rotate-180" : "rotate-0")}
        >
          <path
